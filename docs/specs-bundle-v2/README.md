...
2. Lire `01_doctrine_et_composants.md` : la doctrine miroir appliquee (4 patterns), et la bibliotheque de composants reutilisables — dont le **systeme trace-centre** : `TrackStage` (composant maitre) et `CoachBand` (parole du coach, separee et attribuee).
3. Lire `02_moteur_insights.md` : le **cerveau** de la restitution — ce que l'app peut deduire des donnees RaceBox, classe par profondeur de revelation, avec la frontiere fait/cause (pilote vs coach).
4. Les specs d'ecran sont rangees par bloc dans `specs/`. Chaque fichier couvre un bloc fonctionnel complet.

## Le systeme visuel (fige)

Le **trace schematique du circuit est le langage commun de l'app**. Un pilote pense en virages, pas en ecarts-types : chaque insight se pose **sur le trace, la ou il se produit**. Pattern recurrent decline sur les ecrans data : **`TrackStage`** (le trace + ses modes faisceau / replay / a_vs_b / heatmap) + **`Facts`** (les faits sous le trace) + **`CoachBand`** (le bandeau coach separe, ancre a un point precis, attribue). Voir `01` Pattern 4.
