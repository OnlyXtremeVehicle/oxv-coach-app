...
> **Systeme trace-centre (fige).** La plupart de ces ecrans s'organisent autour d'un
> `[Composant: TrackStage]` : le fait s'inscrit **sur le trace, la ou il se produit** (faisceau,
> replay colorie, superposition a soi). Voir Pattern 4 dans `01`. Quand un coach est lie au
> pilote, sa lecture apparait en `[Composant: CoachBand]` **separe sous le trace**, ancre a un
> point precis, attribue — jamais fondu dans le miroir du pilote.
...
- **But** : situer cette session dans la trajectoire du pilote, dans le temps — et montrer **ou**
  sur le trace l'evolution se joue.
- **Layout** :
  - `[Composant: TrackStage]` mode `a_vs_b` : cette session vs session de reference, **superposees
    sur le meme trace**. La zone de divergence est surlignee — l'ecart devient geographique.
  - `[Composant: TimelineEvolution]` : sessions sur le meme circuit (la tendance dans le temps).
  - `[Composant: MetricHero]` : delta factuel vs reference perso (« +0,4 s vs votre meilleure
    session ici »).
  - Chips de secteur : ou se loge l'ecart (S1 / S2 / S3).
  - `[Composant: CoachBand]` si coach lie : ancre a la zone de divergence.
- **Donnees** : historique perso (Supabase). Survit aux montees de version (V2→V3→V4).
- **Doctrine** : **soi contre soi uniquement** ici. Aucun autre pilote. L'app montre la
  divergence ; elle ne dit pas quoi changer pour la combler (Pattern 4).
...
- **But** : rejouer la trajectoire **empruntee** (relecture) — revivre le tour, pas lire un
  rapport. Pas de ghost « ideal ».
- **Layout** :
  - `[Composant: TrackStage]` mode `replay` : un point parcourt le trace, la trajectoire
    empruntee coloriee par vitesse (vert plein gaz → bleu freinage).
  - `live-readout` synchronise : vitesse / G / temps **au point courant**.
  - Controle de lecture (play / pause / scrub), reperes de secteur sur la barre.
  - `[Composant: CoachBand]` si coach lie : ancre au point lu (ex. « freinage S2 »), synchronise
    au replay.
- **Donnees** : frames du tour (Supabase / cache local).
- **Doctrine** : c'est une **relecture de la realite**. Pas de ligne fantome « parfaite » a
  rattraper, pas de coaching de trajectoire cote app.
