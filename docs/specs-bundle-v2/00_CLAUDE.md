...
### La frontiere fait / cause (systeme trace-centre)
Deux regles figees, materialisees dans les maquettes validees et detaillees dans
`02_moteur_insights.md` :
- **Le fait s'inscrit sur le trace.** La plupart des ecrans data s'organisent autour d'un
  `TrackStage` (le trace schematique) : faisceau de trajectoires, replay colorie, superposition
  a soi. Le chiffre vit la ou il se produit, pas a plat. C'est ce qui rend l'app intuitive sans
  rien diriger.
- **La causalite orientee vit uniquement cote coach.** Cote pilote : un fait revele, meme
  profond (« votre corde au V4 varie de 1,8 m »). Designer la cause a corriger (« fixe un repere »,
  « l'objectif est… ») est reserve au `CoachBand` — separe, ancre, attribue a un tiers agree.
  Reformuler un conseil en constat ne le rend pas conforme si le contenu oriente la conduite.
