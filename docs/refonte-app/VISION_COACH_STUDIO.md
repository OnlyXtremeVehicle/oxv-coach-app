...
- **Autorisé** : une console coach montrant _qui est en piste / aux stands /
  nb de tours_ (factuel, comme `tour-controle`).
- **Proscrit tel quel** : le _ranking par QDI_. Options : (a) garder proscrit,
...
- Nuance : le _coach_ peut voir des choses en direct sur SA tablette (pas le pilote).
...
- **Autorisé** : Smart Flagging _factuel_ (« Virage 2 : plus forte chute de QDI »).
...
- **P4 — Console de direction** : version _factuelle_ (statut, pas ranking — C1).
...
4. **C4 — Notes privées accessibles (RGPD).** Pas de « profil psycho » opaque ;
   ...
   sensibles sans base légale. _(défaut appliqué)_
5. **C5 — Équilibre par proxy lacet assumé.** Pas d'« angle volant » (capteur
   absent) ; estimation lacet vs attendu, dite dans le bloc méthode. _(défaut appliqué)_
