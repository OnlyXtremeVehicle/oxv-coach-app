...
## Pilote professionnel `(pro)` — _fondation FAITE (PR-I)_
...
## Coach `(coach)` — _espace déjà riche_
...
## Partenaire `(partner)` — _marketplace FAITE (PR-F1→F4)_
...
| **B2B Event Report** (participation, qualité, coaching, partenaires, média, conclusion) | V4 §25          | `b2b_event_reports` (oui) + un concept d'**événement** à clarifier | à cadrer — _quel est « l'événement » en base ?_ |

## Admin `(admin)` — _control tower déjà fourni_
