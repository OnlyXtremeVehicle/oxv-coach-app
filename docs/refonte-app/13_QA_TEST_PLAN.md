...
| Test                      | Attendu                                                                                   | Réf                                        |
| ------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------ |
| Lexique prescriptif       | Aucune occurrence de _freinez, corrigez, vous devez, évitez, erreur, mauvais_ côté pilote | `04_DESIGN_CANON §7`, `09_UX_COPY_LIBRARY` |
...
Le service expose `isAvailable()`, `connect/disconnect`, reconnexion auto (`ReconnectPhase = 'idle' | 'reconnecting' | 'lost'`, `RECONNECT_MAX_ATTEMPTS = 5`, déclenchée par `device.onDisconnected`), `forget`, et un statut qui _flappe_ pendant les tentatives. Chaque message d'erreur doit répondre : **que s'est-il passé ? qu'est-ce qui est préservé ? que puis-je faire ?** (`01_ORGANISATION_PRODUIT`, Session).
