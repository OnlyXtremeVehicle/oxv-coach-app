...
function buildPath() {
  ...
  for (const seg of segments) {
    if (seg.type === 'straight') {
      ...
      const r = seg.radius,
        ang = (seg.angle * Math.PI) / 180,
        dir = seg.dir,
        vmax = cornerSpeed(r);
      const arc = r * ang,
        n = Math.max(2, Math.round(arc / step)),
        dp = ang / n;
      for (let i = 0; i < n; i++) {
        heading += dir * dp;
        x += step * Math.cos(heading);
...
function speedProfile(pts, skill) {
  const AB = 9.0 * skill.brake,
    AA = 5.0 * skill.accel;
  const v = pts.map((p) => p.vmax);
  for (let i = pts.length - 2; i >= 0; i--) {
    const ds = pts[i + 1].s - pts[i].s;
    v[i] = Math.min(v[i], Math.sqrt(v[i + 1] ** 2 + 2 * AB * ds));
  }
  for (let i = 1; i < pts.length; i++) {
    const ds = pts[i].s - pts[i - 1].s;
    v[i] = Math.min(v[i], Math.sqrt(v[i - 1] ** 2 + 2 * AA * ds));
  }
  return v;
}
...
function sampleLap(pts, v, lapIndex, skill, t0) {
  const frames = [];
  const totalS = pts[pts.length - 1].s;
  const lat0 = 44.5,
    lon0 = -0.2,
    mLat = 111320,
  ...
  while (sPos < totalS - 0.5) {
    ...
    frames.push({
      lap: lapIndex,
      elapsed_ms: Math.round((t0 + t) * 1000),
      latitude: +(lat0 + Y / mLat).toFixed(7),
      longitude: +(lon0 + X / mLon).toFixed(7),
      speed_ms: +vi.toFixed(3),
      speed_kmh: +(vi * 3.6).toFixed(2),
      heading: +(((((heading * 180) / Math.PI) % 360) + 360) % 360).toFixed(2),
      g_force_x: +gLong.toFixed(3),
      g_force_y: +gLat.toFixed(3),
      ...
      yaw_geom: +yawGeom.toFixed(2),
      corner: p.corner,
      dist_m: +sPos.toFixed(2),
      fix_valid: true,
      pdop: 1.2,
      satellites: 14,
      gps_accuracy_m: 0.9,
...
export function generateSession(nLaps = 6) {
  ...
  for (let l = 0; l < nLaps; l++) {
    ...
    const skill = {
      ...base,
      brake: base.brake * (0.92 + 0.08 * prog),
      accel: base.accel * (0.93 + 0.07 * prog),
    };
    const v = speedProfile(pts, skill);
    const { frames, lapTime } = sampleLap(pts, v, l + 1, skill, t0);
    frames.forEach((f) => allFrames.push(f));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(
    'Frames :',
    frames.length,
    '(~' + Math.round(frames.length / lapTimes.length) + '/tour)'
  );
