...
const docs = [
  { slug: 'pacte', title: 'Pacte de pilotage', file: 'docs/juridique/01_PACTE_DE_PILOTAGE.md' },
  {
    ...
    file: 'docs/juridique/02_CGU_APP_OXV_MIRROR.md',
  },
  {
    slug: 'confidentialite',
    title: 'Politique de confidentialité',
    file: 'docs/juridique/04_POLITIQUE_CONFIDENTIALITE.md',
  },
  {
    slug: 'decharge',
    title: 'Décharge de responsabilité',
    file: 'docs/juridique/05_DECHARGE_RESPONSABILITE.md',
  },
];
...
// `hash` = empreinte SHA-256 du texte EXACT embarque (integrite de la version
// signee, cf. waiverService — evite une dependance crypto native au runtime).
out +=
...
for (const d of docs) {
  ...
  // `version` derivee de l'en-tete « Version X.Y » du markdown : version et
  // empreinte bougent ENSEMBLE (pas de constante decouplee cote service).
  const version = (body.match(/Version\s+(\d+\.\d+)/) || [])[1] || '1.0';
  out +=
    '  ' +
    d.slug +
    ': { slug: ' +
