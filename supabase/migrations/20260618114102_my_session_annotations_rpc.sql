...
-- Appliquee en production le 18 juin 2026 a 11:41:02 (UTC), elle n avait jamais ete
-- versionnee dans ce depot. Source : colonne statements, recollee dans l ordre d execution.
-- Le formatage d origine et les commentaires hors instruction sont perdus ; le SQL, lui,
-- est celui qui a reellement tourne. Ne pas rejouer : deja appliquee.

-- Annotations du coach PARTAGÉES, projetées sur le jour du bilan pilote (miroir de l'objectif).
-- Multiples par séance (une par virage). On réplique explicitement le filtre de visibilité
-- de la RLS pilote (partagé + non supprimé) puisque SECURITY DEFINER la contourne.
create or replace function public.my_session_annotations(p_circuit uuid)
returns table(
  day date, corner_index integer, body text, has_audio boolean, coach_name text, created_at timestamptz
)
